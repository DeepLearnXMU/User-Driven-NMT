...
def _add_dynamic_corpus_opts(parser, build_vocab_only=False):
    ...
    # Cache
    group.add('-train_history', required=True,
                        help="""Path t file containing training history cache.""")
    group.add('-train_current', required=True,
                        help="""Path to file containing training current cache.""")
    group.add('-nearby_history', required=True,
                        help="""Path t file containing nearby users' history cache.""")
    group.add('-far_history', required=True,
                        help="""Path to file containing far users' history cache""")
    group.add('-valid_history', required=True,
                        help="""Path to file containing valid history cache.""")
    group.add('-valid_current', required=True,
                        help="""Path to file containing valid current cache.""")
...
def model_opts(parser):
    ...
    group.add('--contrastive_learning', '-contrastive_learning', action="store_true",
              help='using max-margin loss to do contrastive_learning')
    group.add('--margin', '-margin', type=float, default=1.0,
              help="max margin for triplet loss in contratstive learning.")
    group.add('--c_rate', '-c_rate', type=float, default=1.0,
              help="ratio of triplet loss.")
    group.add('--regular', '-regular', action="store_true",
              help='add regularization that add KL(newtgt, oldtgt) to loss')
    group.add('--r_rate', '-r_rate', type=float, default=1.0,
              help="ratio of KL loss.")
    group.add('-old_tgt',
              help="""Path t file containing old translations from pretrained model""")
...
def _add_train_general_opts(parser):
    ...
    group.add('--fix_pretrain_param', '-fix_pretrain_param',
              type=str, default="none", choices=["none", "all", "encoder"])
...
def translate_opts(parser):
    ...
    # Cache
    group.add('-test_history', required=True,
                        help="""Path to file containing testing history cache.""")
    group.add('-test_current', required=True,
                        help="""Path to file containing testing current cache.""")
