...
def build_loss_compute(model, tgt_field, opt, train=True):
    ...
    if opt.copy_attn:
        ...
        compute = NMTLossCompute(
            ...
            lambda_align=opt.lambda_align, contrastive=opt.contrastive_learning, margin=opt.margin, c_rate=opt.c_rate, regular=opt.regular, r_rate=opt.r_rate)
    compute.to(device)
...
class NMTLossCompute(LossComputeBase):
    ...
    def __init__(self, criterion, generator, normalization="sents",
                 lambda_coverage=0.0, lambda_align=0.0, contrastive=False, margin=1.0, c_rate=1.0,
                 regular=False, r_rate=1.0):
        ...
        if contrastive:
            self.contrastive_loss = nn.TripletMarginLoss(margin=margin, p=2)
            self.c_rate = c_rate

        self.regular = regular
        if regular:
            self.regular_loss = nn.KLDivLoss(reduction='sum')
            self.r_rate = r_rate

    def _make_shard_state(self, batch, output, range_,
                          attns=None, output_nb=None, output_far=None):
        '''
        shard_state = {
            "output_nb": output_nb, 
            "output_far": output_far,
            "old_tgt": old_tgt[range_[0] + 1: range_[1], :, 0],
            "output": output,
            "target": batch.tgt[range_[0] + 1: range_[1], :, 0],
        }
        print ('old_tgt_1', old_tgt.shape)
        print ('batch.tgt', batch.tgt.shape)
        print ('output', output.shape)
        print ('old_tgt_2', shard_state["old_tgt"].size())
        print ('target', shard_state["target"].shape)
        '''
    ...
    def _compute_loss(self, batch, output, target, 
                      output_nb=None, output_far=None, std_attn=None, coverage_attn=None, align_head=None, ref_align=None):
        ...
        if self.lambda_align != 0.0:
            ...
            loss += align_loss
        if self.contrastive:
            assert output_nb is not None
            assert output_far is not None
            bottled_nb_output = self._bottle(output_nb)
            scores_nb = self.generator(bottled_nb_output)
            bottled_far_output = self._bottle(output_far)
            scores_far = self.generator(bottled_far_output)
            
            '''
            dis1 = torch.mean(scores-scores_nb)
            dis1 = torch.norm(dis1, p=2)
            dis2 = torch.mean(scores-scores_far)
            dis2 = torch.norm(dis2, p=2)
            dis = dis1-dis2
            print(dis.item())
            '''

            loss += self.c_rate * self.contrastive_loss(scores, scores_nb, scores_far)
        '''
        if self.regular:
            assert old_tgt is not None
            old_gtruth = old_tgt.view(-1)
            ones = torch.sparse.torch.eye(scores.shape[1]).cuda()
            onehot = ones.index_select(0, old_gtruth)
            print('old_gtruth', old_gtruth.shape)
            print('onehot', onehot, onehot.shape)
            loss += self.r_rate * self.regular_loss(scores, onehot)
        '''
    ...
    def __call__(self,
                ...
                output, 
                attns,
                output_nb=None, output_far=None,
                normalization=1.0,
                shard_size=0,
                trunc_start=0,
                trunc_size=None):
        ...
        #shard_state = self._make_shard_state(batch, output, trunc_range, attns)
        shard_state = self._make_shard_state(batch, output, trunc_range, attns,
                                             output_nb=output_nb, output_far=output_far)
        if shard_size == 0:
            loss, stats = self._compute_loss(batch, **shard_state)
            return loss / float(normalization), stats
        batch_stats = onmt.utils.Statistics()
