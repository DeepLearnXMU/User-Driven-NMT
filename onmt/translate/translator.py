...
class Translator(object):
    ...
    def cache_batch(self, filepath, idxs):
        ...
        src_data = {"reader": self.src_reader, "data": cache_src}
        _cache_readers, _cache_data = inputters.Dataset.config([('src', src_data)])
        cache_data = inputters.Dataset(
            self.fields, readers=_cache_readers, data=_cache_data,
        ...
        for cache in cache_data_iter:
            #cache_idxs = cache.indices.cpu().data.numpy()
            #print('cache.src', cache.src[0].size())
            #print('cache_idxs', cache_idxs)
            return cache.src[0]
    ...
    def translate(
        ...
        Args:
            src: See :func:`self.src_reader.read()`. {side:'src', "indices": i}
...
#             indicating whether to include that example in the dataset. moren wei None
#         fields (dict[str, torchtext.data.Field]): A dict
#             mapping each side to its list of name-Field pairs.
        ...
        history_path = history_cache
        current_path = current_cache
        for batch in data_iter:
            idxs = batch.indices.cpu().data.numpy()
            history_cache = self.cache_batch(history_path, idxs)
            current_cache = self.cache_batch(current_path, idxs)
            batch_data = self.translate_batch(
                batch, data.src_vocabs, attn_debug,
                history_cache, current_cache
            )
    ...
    def _run_encoder(self, batch, history_cache=None, current_cache=None):
        src, src_lengths = batch.src if isinstance(batch.src, tuple) \
                           else (batch.src, None)

        enc_states, memory_bank, src_lengths = self.model.encoder(
            src, src_lengths, history=history_cache, current=current_cache)
    ...
    def _translate_batch_with_strategy(
        ...
        src, enc_states, memory_bank, src_lengths = self._run_encoder(
            batch, history_cache=history_cache, current_cache=current_cache)
        self.model.decoder.init_state(src, memory_bank, enc_states)
        ...
        for step in range(decode_strategy.max_length):
            #print('decode_strategy.current_predictions', decode_strategy.current_predictions)
